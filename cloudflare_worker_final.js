...
async function handleRequest(request) {
  ...
  try {
    ...
    if (request.method === 'GET' && url.pathname === '/database') {
      return new Response(JSON.stringify({
        pc_knowledge_database: PC_KNOWLEDGE_DB,
        stats: {
          total_entries: Object.keys(PC_KNOWLEDGE_DB.cpu).length + 
                        Object.keys(PC_KNOWLEDGE_DB.gpu).length + 
                        Object.keys(PC_KNOWLEDGE_DB.builds).length + 
                        Object.keys(PC_KNOWLEDGE_DB.troubleshooting).length
        }
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' }
      });
